...
<p align="center">
  <img alt="exemplo de Shopping Cart" src="./out.gif">
</p>
...
### 1 - Criar uma listagem de produtos. :heavy_check_mark:
...
```
onde `$QUERY` é o valor da sua busca. Para este trabalho, a busca obrigatória foi o termo `computador`.

O retorno desse _endpoint_ será algo no formato `json`. Exemplo, se for pesquisado "computador":
```json
{
    "site_id": "MLB",
    "query": "computador",
...
```
A lista de produtos a serem exibidos é o _array_ `results` no `JSON` acima.

 * Adicionar o elemento retornado da função `createProductItemElement(product)` como filho do elemento `<section class="items">`.

### 2. Adicionar o produto ao carrinho de compras. :heavy_check_mark:
Cada produto na página _HTML_ possui um botão com o nome `Adicionar ao carrinho!`.

Ao clicar nesse botão, deve ser realizada uma requisição para o _endpoint_:
```javascript
"https://api.mercadolibre.com/items/$ItemID"
```
onde `$ItemID` deve ser o valor `id` do item selecionado.

Quando colocado o id `MLB1341706310`, o retorno desse _endpoint_ será algo no formato:
```JSON
{
    "id": "MLB1341706310",
    "site_id": "MLB",
 ...
 * Utilizar a função `createCartItemElement()` para criar os componentes _HTML_ referentes a um item do carrinho.
 * Adicionar o elemento retornado da função `createCartItemElement(product)` como filho do elemento `<ol class="cart__items">`.

### 3. Remover o item do carrinho de compras ao clicar nele. :heavy_check_mark:
 * Ao clicar no **produto no carrinho de compra**, ele deve ser removido da lista.
 * A função `cartItemClickListener(event)` deve ser implementada com a lógica necessária para realizar a remoção.
  
### 4. Carregar o carrinho de compras através do **LocalStorage** ao iniciar a página. :heavy_check_mark:
...
Criar um botão para remover todos os itens do carrinho de compras. Ele deve, **obrigatóriamente**, ter a classe `empty-cart`.

### 7. Adicione um texto de "loading" durante uma requisição à API. :heavy_check_mark:
Mostrar a palavra "loading..." em algum lugar da página durante a requisição à API. O elemento mostrado durante o carregamento da página deve, **obrigatóriamente**, ter a classe `loading`.
